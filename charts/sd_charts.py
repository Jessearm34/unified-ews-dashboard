...
def safety_profile_table(workers: pd.DataFrame, forms: pd.DataFrame) -> str:
    ...
    for _, r in df.iterrows():
        ...
        bbso_link = f"<span class='num-link' style='cursor:pointer' hx-get='/_sd_person_forms?worker_id={wid}&type=bbso' hx-target='#person-forms-panel' hx-swap='innerHTML'>{bbso_count}</span>" if bbso_count > 0 else "<span class='badge'>0</span>"
        rir_link = f"<span class='num-link warn' style='cursor:pointer' hx-get='/_sd_person_forms?worker_id={wid}&type=rir' hx-target='#person-forms-panel' hx-swap='innerHTML'>{rir_count}</span>" if rir_count > 0 else "<span class='badge'>0</span>"
        rows.append(f"""<tr>
