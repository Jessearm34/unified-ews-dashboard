...
def qb_section(
    ...
    if not ds:
        return {"kpis": [], "charts": {}, "loaded_at": now_iso, "has_more": {}, "section": section, "basis": basis, "range": range}

    start, end = resolve_date_range(range)
    invoices = QB.filter_invoices(ds.invoices, start, end)
    handler = _SECTION_HANDLERS.get(section)
    if handler is None:
        return {"kpis": [], "charts": {}, "loaded_at": now_iso, "has_more": {"qb": True}, "section": section, "basis": basis, "range": range, "error": f"Unknown section: {section}"}
